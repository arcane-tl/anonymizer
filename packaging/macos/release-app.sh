...
echo "  2. Set cask version + sha256 in packaging/homebrew/Casks/anonymizer-app.rb"
echo "     (or: packaging/homebrew/update-for-release.sh --version … --source-sha … --cask-sha …)"
echo "  3. Sync public tap (Formula/ + Casks/ — see packaging/homebrew/README.md):"
echo "       TAP=\"\$(brew --repository arcane-tl/anonymizer)\""
echo "       cp packaging/homebrew/anonymizer.rb \"\$TAP/Formula/anonymizer.rb\""
echo "       cp packaging/homebrew/Casks/anonymizer-app.rb \"\$TAP/Casks/\""
echo "       rm -f \"\$TAP/anonymizer.rb\""
echo "  4. On other Mac: brew update && brew upgrade anonymizer anonymizer-app"
